...
Header & Footer Réutilisables : Création d'une barre de navigation (Navbar) et d'un pied de page (Footer) professionnels en utilisant les composants Bootstrap. Ces éléments sont gérés via des "partials" Twig ({% include %}) pour être facilement maintenables et présents sur toutes les pages.

Carrousel d'Images : Intégration d'un carrousel Bootstrap fonctionnel en haut de la page pour un impact visuel.

Affichage Dynamique des Catégories : La page d'accueil interroge la base de données via le HomeController et CategorieRepository pour afficher dynamiquement la liste des catégories de véhicules disponibles sous forme de cartes.

2. Pages de Catégories par Véhicule
Routing Dynamique : Mise en place de routes dynamiques avec des paramètres (ex: /categorie/{id}).

Affichage Spécifique : Création d'un CategorieController qui, grâce au ParamConverter de Symfony, récupère automatiquement une catégorie via son ID et affiche une page listant uniquement les véhicules associés à cette catégorie.

Lien Logique : La page d'accueil et les pages de catégories sont interconnectées, permettant une navigation fluide pour l'utilisateur.

3. Intégration Front-End
Bootstrap 5 : Le projet est entièrement stylisé avec Bootstrap 5, installé et géré via importmap et AssetMapper (sans Node.js).

Font Awesome : La bibliothèque d'icônes Font Awesome a été intégrée pour une utilisation facile des icônes (par exemple, dans le footer).

CSS Personnalisé : Utilisation de CSS personnalisé pour des ajustements fins (effets de survol, espacements sur mesure) tout en respectant la logique de Bootstrap.
